...
// GET — fetch all acquisition costs for a property
...
// POST — upsert (save all costs for a property in one call)
export async function POST(request: NextRequest) {
  ...
  // Replace all costs for this property
  await adminSupabase.from('property_acquisition_costs').delete().eq('property_id', propertyId)
  ...
  if (costs.length > 0) {
    ...
    const { error } = await adminSupabase.from('property_acquisition_costs').insert(rows)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  }
